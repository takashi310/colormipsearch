...
public class MaximumFilter {
    ...
    public static <T extends IntegerType<T>> Img<T> MaximumFilterSphereMT(Img<T> input, int xy_radius, int z_radius, int thread_num) {
        ...
        final Thread[] threads = newThreadArray(thread_num);
        for (int ithread = 0; ithread < threads.length; ithread++) {
